...
#pragma pack(pop)
		...
		implementation struct Antares_Link_Implementation:public Link_Implementation<MasterType,INHERIT(Antares_Link_Implementation)>
		{
...
//				//_link_density_ratio_layer=Allocate_New_Layer< typename MasterType::type_of(canvas),NT,Target_Type< NULLTYPE,Antares_Layer<type_of(link_density_ratio_layer)>*, string& > >(string("Link Density Ratio"));
			...
			static bool submit_attributes(Antares_Link_Implementation* _this,std::vector<string>& bucket)
			{
				// not supported yet
				return true;
			}
...
#pragma pack(pop)
			...
			static bool fetch_attributes(Antares_Link_Implementation* _this,std::vector<string>& bucket)
			{
				_this->plot_link_moe();
		...
		template<typename MasterType,typename InheritanceList>
		int Antares_Link_Implementation<MasterType,InheritanceList>::base_height = 700;
		template<typename MasterType,typename InheritanceList>
		float Antares_Link_Implementation<MasterType,InheritanceList>::_vehicle_length = 5280/220; // space occupied by a vehicle under jam density
		template<typename MasterType,typename InheritanceList>
		float Antares_Link_Implementation<MasterType,InheritanceList>::_link_shift = 20;
	}
}
