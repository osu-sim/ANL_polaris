#pragma once
///----------------------------------------------------------------------------------------------------
/// Execution_Object_Methods.h - Top level execution object which records type_id, revision, and memory information
///----------------------------------------------------------------------------------------------------

#include "Execution_Component_Manager_Methods.h"
...
namespace polaris
{
	...
	*/
	...
	void Execution_Object::Load_Event(void (*p_event)(DataType*,Event_Response&),int start_iteration,int start_sub_iteration)
	{
	...
	void Execution_Object::Reschedule(int start_iteration,int start_sub_iteration)
	{
		...
		if( _world->Is_Running() )
		{
			...
			// Update the TEX
			//DataType::component_manager->Update_Schedule(update_revision);
			((typename DataType::Component_Manager_Type*)_execution_block->component_manager())->Update_Schedule(update_revision);
